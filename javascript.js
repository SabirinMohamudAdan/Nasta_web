...
//         const productCard = document.createElement('div');
//         productCard.className = 'bg-white border border-gray-200 rounded-md shadow-md hover:shadow-lg transition product-card fade-in';
//         productCard.dataset.id = product.id;
//         productCard.style.setProperty('--i', index);
...
function renderProducts() {
    ...
    filteredProducts.forEach((product, index) => {
        ...
        productCard.innerHTML = `
            <img class="w-full h-40 sm:h-48 object-cover rounded-t-md" src="${product.image}" alt="${product.name}">
            <div class="p-4">
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="font-bold text-lg">${product.name}</h3>
